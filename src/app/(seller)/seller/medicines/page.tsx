...
interface Medicine {
  id: string;
  name: string;
  category: { name: string } | string; // এপিআই রেসপন্স অনুযায়ী
  price: number;
  stock: number;
  image?: string;
}
...
export default function SellerMedicines() {
  ...
  const [formData, setFormData] = useState({
    ...
    sellerId: "1a06f77b-b71f-4cf8-9f15-f14badc61128", // আপনার এপিআই থেকে পাওয়া আইডি
  });
  ...
  const fetchMedicines = async () => {
    ...
    try {
      ...
      // আপনার এপিআই রেসপন্স এখন response.data তে আসছে
            ...
            {loading ? (
              ...
              filteredMeds.map((med) => (
                <tr key={med.id} className="hover:bg-white/[0.03] transition-all group">
                  ...
                  <td className="px-10 py-6 font-bold text-slate-400 text-xs italic uppercase">
                    {/* ক্যাটাগরি অবজেক্ট কি না চেক করে নাম দেখানো */}
                    {typeof med.category === 'object' ? med.category.name : med.category}
                  </td>
