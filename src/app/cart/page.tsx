...
export default function CartPage() {
  ...
  const freeShippingThreshold = 1000; // ১০০০ টাকার উপরে ফ্রি শিপিং
    ...
    return (
      <div className="min-h-screen bg-[#020d0a] bg-gradient-to-br from-[#020d0a] via-[#051a14] to-[#10b981]/5 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <h2 className="text-5xl md:text-7xl font-black text-white italic uppercase mb-8 tracking-tighter">
            Cart is <span className="text-emerald-500">Empty</span>
          </h2>
          <Link href="/shop" className="bg-emerald-500 text-black px-12 py-5 rounded-2xl font-black uppercase italic text-xs hover:scale-110 transition-all inline-block shadow-2xl shadow-emerald-500/20">
            Browse Medicines
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#020d0a] bg-gradient-to-br from-[#020d0a] via-[#051a14] to-[#10b981]/10 text-white py-12 md:py-20 px-6 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <motion.div 
          ...
          className="mb-16 bg-white/[0.03] border border-white/5 p-8 rounded-[40px] backdrop-blur-3xl"
        >
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-500">Shipping Status</p>
        ...
        <div className="flex justify-between items-end mb-16">
          <div>
            ...
            <p className="text-emerald-500/30 font-black uppercase text-[10px] tracking-[0.4em] flex items-center gap-2">
               <ShieldCheck size={14} /> Securely managed items
            </p>
          </div>
              ...
              {cart.map((item) => (
                ...
                >
                  ...
                  <div className="flex-1 text-center md:text-left">
                    ...
                    <div className="flex items-center justify-center md:justify-start gap-8">
                      <div className="flex items-center gap-6 bg-black/60 rounded-[25px] p-2 border border-white/5 shadow-2xl">
                        <button onClick={() => updateQuantity(item.medicineId, -1)} className="w-12 h-12 flex items-center justify-center bg-white/5 rounded-2xl hover:bg-emerald-500 hover:text-black transition-all">
                          <Minus size={20} />
                        </button>
                  ...
                  <div className="flex flex-row md:flex-col items-center md:items-end justify-between w-full md:w-auto pt-8 md:pt-0 border-t md:border-none border-white/5">
                    <button onClick={() => removeItem(item.medicineId)} className="text-white/10 hover:text-red-500 p-4 transition-all hover:scale-125">
                      <Trash2 size={28} />
                    </button>
                    <div className="text-right">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white/[0.03] border border-white/10 rounded-[60px] p-12 backdrop-blur-3xl sticky top-24 shadow-2xl overflow-hidden">
              ...
              <h2 className="text-3xl font-black italic uppercase tracking-tighter mb-12 pb-6 border-b border-white/5">
                 Final <span className="text-emerald-500">Bill</span>
              </h2>
              
              <div className="space-y-6 mb-12">
                <div className="flex justify-between items-center text-white/40">
                   <span className="text-[10px] font-black uppercase tracking-[0.2em]">Items Cost</span>
                ...
                <div className="flex justify-between items-end pt-4">
                  <div>
                    <p className="text-[10px] font-black uppercase text-emerald-500 tracking-[0.2em] mb-1">Total Payable</p>
              ...
              <Link href="/shop" className="block text-center mt-10 text-[10px] font-black uppercase tracking-[0.4em] text-white/20 hover:text-emerald-500 transition-all">
                ← Return to Shop
              </Link>
            </div>
          </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-[#020d0a]/90 backdrop-blur-2xl border-t border-white/5 p-8 z-50 flex items-center justify-between">
        <div>
          <p className="text-[9px] font-black uppercase text-white/30 tracking-widest mb-1">Total Due</p>
          <p className="text-3xl font-black italic text-emerald-500">৳{total >= freeShippingThreshold ? total : total + 60}</p>
        </div>
        <button onClick={() => router.push('/customer/checkout')} className="bg-emerald-500 text-black px-10 py-5 rounded-2xl font-black uppercase italic text-xs tracking-widest shadow-xl shadow-emerald-500/20">
          Checkout
        </button>
      </div>
    </div>
