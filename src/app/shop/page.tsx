...
export default function ShopPage() {
  ...
  const addToCart = (med: Medicine) => {
    ...
    if (existing) {
      ...
      cart.push({
        ...
        name: med.name,
        price: med.price,
        image: med.image, // ইমেজ পাথ এখানেও পাস করলাম
        quantity: 1,
      });
    }
  ...
  return (
    <div className="pb-32 selection:bg-emerald-500/30 min-h-screen bg-[#050a0a]">
      <div className="container mx-auto px-4 lg:px-8 max-w-7xl">
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-[450px] bg-white/5 animate-pulse rounded-[3rem]" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8">
            <AnimatePresence>
              {filteredMeds.map((med) => (
                ...
                >
                  ...
                  {/* IMAGE AREA - কার্ডের অর্ধেক অংশ জুড়ে */}
                  <div className="relative h-64 w-full bg-black/20 overflow-hidden border-b border-white/5">
                    ...
                    {/* View Details Icon - ইমেজের নিচে বাম দিকে */}
                    <Link 
                      href={`/shop/${med.id}`} 
                      className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-md text-white p-3 rounded-2xl opacity-0 group-hover:opacity-100 transition-all hover:bg-emerald-500 hover:text-black flex items-center gap-2 text-[10px] font-bold"
                    >
                  ...
                  <div className="p-7 flex flex-col flex-1">
                    <div className="mb-6">
                      <h3 className="font-black text-xl text-white group-hover:text-emerald-400 transition-colors uppercase italic tracking-tight">
                        {med.name}
                      </h3>
                    ...
                    <div className="mt-auto flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="p-4 bg-emerald-500 text-black rounded-2xl hover:bg-white hover:scale-110 transition-all shadow-lg active:scale-90 disabled:opacity-20"
                      >
