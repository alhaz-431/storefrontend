...
export default function ShopPage() {
  ...
  // ✅ safer localStorage + no spam toast
  const addToCart = (med: Medicine) => {
    if (typeof window === "undefined") return;

    const cart: any[] = JSON.parse(
      localStorage.getItem("medistore_cart") || "[]"
    );
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-64 bg-white/5 animate-pulse rounded-[32px]"
            />
          ))}
        </div>
      ) : (
              ...
              {filteredMeds.map((med) => (
                ...
                >
                  ...
                  <div className="h-40 bg-emerald-500/10 rounded-2xl mb-6 flex items-center justify-center">
                    <span className="text-[10px] font-black uppercase text-emerald-500/50 tracking-[0.5em]">
                      No Image
                    </span>
                  </div>
                  ...
                  <p className="text-[9px] text-slate-500 uppercase font-black tracking-widest mb-4 italic">
